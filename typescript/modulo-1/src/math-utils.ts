...
export const calcularMedia = (notas: number[]): number | null => {
    if (notas.length === 0) {
        return null;
    }
 ...
 */
export const calcularMediana = (notas: number[]): number | null => {
    if (notas.length === 0) {
        return null;
    }
...
export const filtrarAtipicos = (notas: number[]): number[] => {
